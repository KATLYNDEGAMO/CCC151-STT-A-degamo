...
def add_student():
    ...
    if course_data:
        cursor.execute("INSERT INTO students VALUES (?, ?, ?, ?, ?)",
                       (student_id, student_name, gender, year_level, course_code))
        conn.commit()
        ...
        # Course does not exist, display an error message
...
def edit_student():
    ...
    cursor.execute("UPDATE students SET Name=?, Gender=?, Year_Level=?, Course_Code=? WHERE ID=?",
                   (student_name, gender, year_level, course_code, student_id))
    conn.commit()
...
def list_students():
    ...
    for student in student_data:
        ...
        dataoutput.insert(tk.END, f"ID: {student_id}, Name: {student_name}, Gender: {gender}, Year Level: {year_level}, Course Code: {course_code}\n")
